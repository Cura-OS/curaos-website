...
#     [--releases-url URL] [--lang TAG] [--dir ltr|rtl]
set -euo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

CONTENT_DIR="$(resolve_content_dir "$(parse_flag content-dir "$@")")"
OUT_DIR="$(parse_flag out "$@")"; OUT_DIR="${OUT_DIR:-${REPO_ROOT}/site}"
DOCS_URL="$(parse_flag docs-url "$@")"; DOCS_URL="${DOCS_URL:-https://curaos-docs.abualruz.com}"
DEMO_URL="$(parse_flag demo-url "$@")"; DEMO_URL="${DEMO_URL:-https://curaos-demo.abualruz.com}"
