...
const CONTENT: SiteContent = {
    ...
    groups: [
      ...
      },
      {
        ...
        apps: [
          {
            name: 'My tasks',
            blurb: 'Personal task management.',
            href: 'https://my-tasks.abualruz.com',
          },
        ],
      },
    ],
  ...
  getStarted: {
    ...
    lines: [
      '$ git clone https://github.com/Cura-Care-Oriented-Stack/curaos',
      'ok: neutral core up',
    ],
  },
  quickstart: {
...
describe('renderPage: link injection', () => {
  ...
  test('renders the demo CTA as a NON-navigational placeholder when not live', () => {
    ...
    expect(html).toMatch(
      /<span class="btn[^"]*" role="link" aria-disabled="true" data-status="coming-soon"/,
    );
  });
  ...
  test('renders the demo CTA as a real live link when demoLive is true', () => {
    ...
    expect(html).toMatch(
      /<a class="btn btn-ghost" href="https:\/\/demo\.curaos\.example">Live demo<\/a>/,
    );
  });
});
...
describe('renderPage: 8-section dev-platform layout', () => {
  ...
  test('resolves footer href tokens from build-time link targets (single source of truth)', () => {
    ...
    expect(html).toContain(`href="https://github.com/Cura-Care-Oriented-Stack/curaos/releases"`); // {releasesUrl}
  ...
  test('still rejects a non-http(s) scheme after footer token substitution', () => {
    const evil: SiteContent = {
      ...
      footer: {
        ...CONTENT.footer,
        columns: [{ heading: 'Bad', links: [{ label: 'x', href: 'javascript:alert(1)' }] }],
      },
    };
...
describe('renderPage: content + air-gap safety', () => {
  ...
  test('rejects a non-http(s) / javascript: link scheme', () => {
    expect(() => renderPage(CONTENT, { ...LINKS, docsUrl: 'javascript:alert(1)' }, LTR)).toThrow(
      /unsafe or non-navigational/,
    );
  });
